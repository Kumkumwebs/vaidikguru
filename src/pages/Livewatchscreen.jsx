...
const avatarUrl = (name) =>
  `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=7A1E1E&color=fff&size=64`;
...
const resolveImg = (path) => {
  ...
  if (clean.startsWith(RAW_HOST)) {
    return `${API}/${clean.slice(RAW_HOST.length).replace(/^\/+/, "")}`;
  }
          ...
          {gifts.map((g, i) => (
            <button key={g._id} onClick={() => setSel(i)}
              ...
              <div className="lw-gift-icon-wrap">
                {gImg(g)
                  ? <img src={gImg(g)} alt={gName(g)} className="w-100 h-100" style={{ objectFit: "cover" }}
                    onError={(e) => { e.target.style.display = "none"; }} />
                  : <span style={{ fontSize: 22 }}>{gEmoji(g)}</span>}
              </div>
          ...
          {loadingLives ? (
            ...
            <>
              ...
              <div className="lw-leave-list">
                {otherLives.map((a) => (
                  <button key={a.astro_id} onClick={() => onJoinOther(a)} className="lw-leave-item">
                    <div className="lw-leave-item-avatar-wrap">
                      <img src={resolveImg(a.profile_image)} alt={a.name}
                        className="lw-leave-item-avatar"
                        onError={(e) => { e.target.src = avatarUrl(a.name); }} />
  ...
  if (msg.isGift) {
    return (
      <div className="lw-msg-row">
        <img src={avatarUrl(msg.name)} alt={msg.name} className="lw-msg-avatar" />
        <div className="flex-grow-1 min-w-0">
          <div className="lw-msg-top">
            <p className="lw-msg-name mb-0">{msg.name}</p>
            <span className="lw-msg-time">{fmtTs(msg.date_time)}</span>
          </div>
          <div className="lw-msg-gift-row">
            {msg.giftImg ? <img src={msg.giftImg} className="lw-msg-gift-img" alt={msg.giftName} /> : <span style={{ fontSize: 18 }}>{msg.giftEmoji}</span>}
            <span className="lw-msg-text mb-0">gifted <b>{msg.giftName}</b></span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="d-flex flex-column align-items-center justify-content-center gap-3 px-4 text-center h-100 w-100">
      <div className="lw-placeholder-avatar-wrap">
        {img && !err
          ? <img src={img} alt={name} onError={() => setErr(true)} className="lw-placeholder-avatar" />
          : <div className="lw-placeholder-avatar-letter">{name.charAt(0)}</div>}
        ...
        {lives.map((a) => (
          <button key={a.astro_id} onClick={() => onJoin(a)} className="lw-strip-item">
            <div className="lw-strip-avatar-wrap">
              ...
              <img
                src={resolveImg(a.profile_image) || avatarUrl(a.name)}
                alt={a.name}
...
export default function LiveWatchScreen() {
    ...
    try {
      ...
      if (data?.status && Array.isArray(data.data)) {
        ...
        for (const item of data.data) {
          ...
          // Dedupe by astrologer, not just channel_id — the API can
          // return several live entries (different channel_id) for the
          // same astrologer if a prior session wasn't closed out cleanly.
          // Without this, "Also Live Now" showed the same person's card
          // repeated N times instead of once.
          const astroKey = id || `${item.astrologer_id?.displayname || ""}_${item.astrologer_id?.profile_img || ""}`;
        ...
        log("warn", `listing_of_live_astrlogers: unexpected shape ${JSON.stringify(data)?.slice(0, 200)}`);
      }
    } catch (err) {
      log("warn", `listing_of_live_astrlogers failed: ${err?.message}`);
    }
    ...
    if (!handled) {
      try {
        ...
        if (data2?.status && Array.isArray(data2.results)) {
          setOtherLives(
            data2.results
              .filter((a) => String(a.astro_id) !== String(astroId))
              .slice(0, 15)
          );
        } else {
          log("warn", `get_live_astrologer: unexpected shape ${JSON.stringify(data2)?.slice(0, 200)}`);
        }
      } catch (err) {
        log("warn", `get_live_astrologer fallback failed: ${err?.message}`);
      }
    }

    setLoadingLives(false);
  }, [astroId, channelId, log]);
  ...
  const onGiftSent = (result) => {
    ...
    const payload = {
      ...
      giftName: gName(g), giftEmoji: gEmoji(g), giftImg: gImg(g), message_id: msgRef.key,
    };
    set(msgRef, payload).catch(() => { });
    ...
    <div className={`lw-root${isMaximized ? " lw-maximized" : ""}`}>
      ...
      {!isMaximized && (
        <div className="lw-header">
          ...
          {astroImage && !imgErr
            ? <img src={astroImage} alt={astroName} onError={() => setImgErr(true)} className="lw-avatar" />
            : <div className="lw-avatar-fallback">{astroName.charAt(0)}</div>}
        ...
        {!isMaximized && (
          <div className="col-lg-auto col-12 lw-col-side">
            <div className="d-lg-block d-flex justify-content-between gap-2 w-100 px-lg-0 px-5">
              ...
              <SideBtn color={followed ? DQ_MAROON : DQ_MUTED} label={followed ? "Following" : "Follow"} onClick={() => setFollowed(f => !f)}
